...
import com.invoice_reader.invoice_reader.banque.repository.AccountingEntryDao;
...
import com.invoice_reader.invoice_reader.achat.service.AchatInvoiceProcessingService;
...
@Slf4j
@CrossOrigin(origins = "*")
public class AchatInvoiceController {

    private final AchatInvoiceDao dynamicInvoiceDao;
    private final AchatInvoiceProcessingService processingService;
    ...
    public ResponseEntity<?> uploadAndProcessBatch(
        ...
        for (MultipartFile file : files) {
            ...
            try {
                ...
                AchatInvoice processed = sessionUser.isClient()
    ...
    public ResponseEntity<?> reprocess(
        ...
        return dynamicInvoiceDao.findById(id)
                .map(invoice -> {
                    ...
                    try {
                        AchatInvoice processed = processingService.reprocessExistingInvoice(invoice);
                        return ResponseEntity.ok(toResponse(processed));
                    } catch (ExercisePeriodException e) {
                        String message = String.format(
                                "Facture hors période d'exercice (du %s au %s). Date facture: %s",
    ...
    public ResponseEntity<?> bulkProcess(
        ...
        for (Long id : ids) {
            ...
            item.put("id", id);
            try {
                Optional<AchatInvoice> invoiceOpt = dynamicInvoiceDao.findById(id);
                ...
                AchatInvoice invoice = invoiceOpt.get();
    ...
    public ResponseEntity<?> stats(
        ...
        List<AchatInvoice> lowConfidence = filterByClientValidated
    ...
    public ResponseEntity<?> getFile(
        ...
        Optional<AchatInvoice> invoiceOpt = dynamicInvoiceDao.findById(invoiceId);
        if (invoiceOpt.isEmpty()) {
...
@PostMapping("/{id}/link-tier")
    ...
    public ResponseEntity<?> linkTier(
        ...
        try {
            ...
            AchatInvoice invoice = dynamicInvoiceDao.findById(id)
    ...
    private AchatInvoice createUploadOnlyInvoice(MultipartFile file) {
    ...
    private boolean canAccessInvoiceInDossier(SessionUser sessionUser, AchatInvoice invoice, Long dossierId) {
    ...
    private boolean canAccessInvoice(SessionUser sessionUser, AchatInvoice invoice) {
    ...
    private Map<String, Object> toResponse(AchatInvoice invoice) {
    ...
    private Map<String, Object> toDetailedResponse(AchatInvoice invoice) {
    ...
    private Tier convertDtoToEntity(com.invoice_reader.invoice_reader.account_tier.dto.TierDto dto) {
        return Tier.builder()
                .id(dto.getId())
