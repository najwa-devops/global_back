...
public class AchatFieldExtractorService {

    private final TextCleaningService textCleaningService;

    public AchatExtractionResult extractWithTemplate(String ocrText, AchatTemplate template) {
        long start = System.currentTimeMillis();
        String preparedText = prepareOcrText(ocrText);

        Map<String, AchatExtractionResult.ExtractedField> extracted = new LinkedHashMap<>();
        List<String> missing = new ArrayList<>();
        List<String> lowConfidence = new ArrayList<>();

        for (AchatFieldDefinitionJson field : template.getFieldDefinitions()) {
            ...
            extracted.put(field.getFieldName(),
                    AchatExtractionResult.ExtractedField.builder()
                            .value(attempt.value)
        ...
        return AchatExtractionResult.builder()
    ...
    public AchatExtractionResult extractWithoutTemplate(String ocrText) {
        long start = System.currentTimeMillis();
        String preparedText = prepareOcrText(ocrText);
        ...
        if (!tvaRates.isEmpty()) {
            extracted.put("tvaRate", AchatExtractionResult.ExtractedField.builder()
                    ...
                    .build());
            if (tvaRates.size() >= 2) {
                extracted.put("tvaRate2", AchatExtractionResult.ExtractedField.builder()
        ...
        if (extracted.containsKey("tva2") && extracted.containsKey("tva")) {
            AchatExtractionResult.ExtractedField tvaField = extracted.get("tva");
        ...
        try {
            ...
            AchatExtractionResult.ExtractedField amountTtcField = extracted.get("amountTTC");
    ...
    private void extractAndAdd(
            Map<String, AchatExtractionResult.ExtractedField> extracted,
            List<String> missing,
            String fieldName,
            String text,
    ...
    private Double getExtractedAmount(
            Map<String, AchatExtractionResult.ExtractedField> extracted,
            String fieldName) {
        AchatExtractionResult.ExtractedField field = extracted.get(fieldName);
        if (field == null) {
            return null;
        }
    ...
    private Double findReasonableTvaCandidate(
            Map<String, AchatExtractionResult.ExtractedField> extracted,
            double amountTTC) {
        AchatExtractionResult.ExtractedField tvaValuesField = extracted.get("tvaValues");
    ...
    private void enforcePreferredInvoiceNumber(
        ...
        AchatExtractionResult.ExtractedField existingField = extracted.get("invoiceNumber");
    ...
    private ExtractionAttempt extractField(String text, AchatFieldDefinitionJson def) {
     ...
     * bloc/ligne
     */
    private void extractUnifiedIdentifiers(String text, Map<String, AchatExtractionResult.ExtractedField> extracted) {
