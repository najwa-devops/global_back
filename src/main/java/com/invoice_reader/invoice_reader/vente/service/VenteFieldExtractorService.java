...
public class VenteFieldExtractorService {
    private static final List<String> SALES_ICE_PATTERNS = mergePatterns(
            VenteExtractionPatterns.ICE_PATTERNS,
            ExtractionPatterns.ICE_PATTERNS);
    private static final List<String> SALES_IF_PATTERNS = mergePatterns(
            VenteExtractionPatterns.IF_PATTERNS,
            ExtractionPatterns.IF_PATTERNS);
    private static final List<String> SALES_RC_PATTERNS = mergePatterns(
            VenteExtractionPatterns.RC_PATTERNS,
            ExtractionPatterns.RC_PATTERNS);

    public VenteExtractionResult extractWithTemplate(String ocrText, AchatTemplate template) {
        long start = System.currentTimeMillis();

        Map<String, VenteExtractionResult.ExtractedField> extracted = new LinkedHashMap<>();
        ...
        for (AchatFieldDefinitionJson field : template.getFieldDefinitions()) {
            ...
            extracted.put(field.getFieldName(),
                    VenteExtractionResult.ExtractedField.builder()
                            .value(attempt.value)
    ...
    public VenteExtractionResult extractWithoutTemplate(String ocrText) {
        long start = System.currentTimeMillis();
        ...
        VenteExtractionResult.ExtractedField htField = extracted.get("amountHT");
        VenteExtractionResult.ExtractedField tvaField = extracted.get("tva");
        VenteExtractionResult.ExtractedField ttcField = extracted.get("amountTTC");
    ...
    private void extractAndAdd(
            Map<String, VenteExtractionResult.ExtractedField> extracted,
            List<String> missing,
            String fieldName,
            String text,
    ...
    private Double parseAmountField(VenteExtractionResult.ExtractedField field) {
        if (field == null) {
            return null;
        }
     ...
     * bloc/ligne
     */
    private void extractUnifiedIdentifiers(String text, Map<String, VenteExtractionResult.ExtractedField> extracted) {
