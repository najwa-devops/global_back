package com.invoice_reader.invoice_reader.vente.controller;
...
import com.invoice_reader.invoice_reader.vente.service.VenteInvoiceProcessingService;
...
@Slf4j
@CrossOrigin(origins = "*")
public class VenteInvoiceController {

    private final VenteInvoiceRepository salesInvoiceRepository;
    private final VenteInvoiceProcessingService processingService;
    ...
    public ResponseEntity<?> uploadAndProcessBatch(
        ...
        for (MultipartFile file : files) {
            ...
            try {
                ...
                VenteInvoice processed = sessionUser.isClient()
    ...
    public ResponseEntity<?> reprocess(
        ...
        return salesInvoiceRepository.findById(id)
                .map(invoice -> {
                    ...
                    try {
                        VenteInvoice processed = processingService.reprocessExistingInvoice(invoice);
                        return ResponseEntity.ok(toResponse(processed));
                    } catch (ExercisePeriodException e) {
                        String message = String.format(
                                "Facture hors période d'exercice (du %s au %s). Date facture: %s",
    ...
    public ResponseEntity<?> salesJournal(
        ...
        List<VenteInvoice> invoices = salesInvoiceRepository.findByDossierIdOrderByCreatedAtDesc(dossierId);
        List<Map<String, Object>> entries = new ArrayList<>();

        for (VenteInvoice invoice : invoices) {
    ...
    public ResponseEntity<?> bulkProcess(
        ...
        for (Long id : ids) {
            ...
            item.put("id", id);
            try {
                Optional<VenteInvoice> invoiceOpt = salesInvoiceRepository.findById(id);
                ...
                VenteInvoice invoice = invoiceOpt.get();
    ...
    public ResponseEntity<?> stats(
        ...
        List<VenteInvoice> lowConfidence = filterByClientValidated
    ...
    public ResponseEntity<?> getFile(
        ...
        Optional<VenteInvoice> invoiceOpt = salesInvoiceRepository.findById(invoiceId);
        if (invoiceOpt.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        VenteInvoice invoice = invoiceOpt.get();
    ...
    public ResponseEntity<?> linkTier(
        ...
        try {
            VenteInvoice invoice = salesInvoiceRepository.findById(id)
    ...
    private VenteInvoice createUploadOnlyInvoice(MultipartFile file) {
    ...
    private boolean canAccessInvoiceInDossier(SessionUser sessionUser, VenteInvoice invoice, Long dossierId) {
    ...
    private boolean canAccessInvoice(SessionUser sessionUser, VenteInvoice invoice) {
    ...
    private Map<String, Object> toResponse(VenteInvoice invoice) {
    ...
    private boolean isPendingWorkflowInvoice(VenteInvoice invoice) {
    ...
    private Map<String, Object> toDetailedResponse(VenteInvoice invoice) {
