...
public class BanqueReleve {
    ...
    @Convert(converter = BanqueStatusConverter.class)
    private BanqueStatus status = BanqueStatus.PENDING;
    ...
    private List<BanqueTransaction> transactions = new ArrayList<>();
    ...
    public boolean isModifiable() {
        return status != BanqueStatus.VALIDATED && status != BanqueStatus.COMPTABILISE;
    }

    public boolean canBeValidated() {
        return status == BanqueStatus.READY_TO_VALIDATE ||
                status == BanqueStatus.TREATED;
    }
    ...
    public void calculateTotalsFromTransactions() {
        ...
        this.totalDebit = transactions.stream()
                .filter(this::includeInStatementTotals)
                .map(BanqueTransaction::getDebit)
                .filter(d -> d != null)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }

    private boolean includeInStatementTotals(BanqueTransaction transaction) {
        if (transaction == null) {
            return false;
        }
    ...
    public void addTransaction(BanqueTransaction transaction) {
