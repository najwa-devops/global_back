...
import com.invoice_reader.invoice_reader.banque.service.BanqueTransactionAccountLearningService;
...
public class BanqueTransactionController {
    ...
    private final BanqueTransactionRepository repository;
    ...
    public ResponseEntity<?> getByStatement(@PathVariable("statementId") Long statementId) {
        List<BanqueTransaction> transactions = repository.findByStatementIdOrderByTransactionIndexAsc(statementId);
    ...
    public ResponseEntity<?> search(
        ...
        List<BanqueTransaction> transactions;
    ...
    public ResponseEntity<?> update(
        ...
        return repository.findByIdWithStatement(id)
                .map(transaction -> {
                    ...
                    if (transaction.getStatement().getStatus() == BanqueStatus.TREATED) {
                        transaction.getStatement().setStatus(BanqueStatus.TREATED);
                    }

                    BanqueTransaction saved = repository.save(transaction);
    ...
    public ResponseEntity<?> create(@Valid @RequestBody CreateBankTransactionRequest request) {
        BanqueReleve statement = statementRepository.findById(request.getStatementId())
                .orElse(null);
        if (statement == null) {
        ...
        BanqueTransaction saved = repository.save(tx);
    ...
    public ResponseEntity<?> bulkUpdate(@RequestBody Map<String, Object> request) {
        ...
        for (Long id : ids) {
            Optional<BanqueTransaction> opt = repository.findByIdWithStatement(id);
            ...
            BanqueTransaction transaction = opt.get();
    ...
    public ResponseEntity<?> reindex(@PathVariable Long statementId) {
        List<BanqueTransaction> txs = repository.findByStatementIdOrderByTransactionIndexAsc(statementId);
        int idx = 0;
    ...
    private Map<String, Object> toResponse(BanqueTransaction transaction) {
    ...
    private String buildForbiddenTransactionMessage(BanqueStatus status, String action) {
        if (status == BanqueStatus.COMPTABILISE) {
            return "Relevé comptabilisé, " + action + " impossible";
        }
        if (status == BanqueStatus.VALIDATED) {
    ...
    private boolean resolveEffectiveCmApplied(BanqueTransaction transaction) {
        if (transaction == null) {
            return false;
        }
    ...
    private void refreshStatementAggregates(Long statementId) {
        ...
        BanqueReleve statement = statementRepository.findById(statementId).orElse(null);
        if (statement == null) {
            return;
        }
