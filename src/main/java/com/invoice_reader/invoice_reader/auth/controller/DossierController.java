...
import com.invoice_reader.invoice_reader.auth.dto.CreateDossierRequest;
...
import com.invoice_reader.invoice_reader.database.dao.UserAccountDao;
...
public class DossierController {
    ...
    public ResponseEntity<?> listDossiers(HttpSession session) {
        ...
        List<Map<String, Object>> response = dossiers.stream()
                .map(d -> {
                    ...
                    long validatedBankStatementsCount = bankStatementRepository.countByDossierIdAndStatus(d.getId(), BanqueStatus.VALIDATED)
                            + bankStatementRepository.countByDossierIdAndStatus(d.getId(), BanqueStatus.COMPTABILISE);
    ...
    public ResponseEntity<?> deleteDossier(@PathVariable Long id, HttpSession session) {
        ...
        List<BanqueReleve> statements = bankStatementRepository.findByDossierIdOrderByCreatedAtDesc(id);
        for (BanqueReleve stmt : statements) {
    ...
    private boolean isPendingInvoice(AchatInvoice invoice) {
    ...
    private boolean isPendingInvoice(com.invoice_reader.invoice_reader.vente.entity.VenteInvoice invoice) {
        if (invoice == null || invoice.getStatus() == null) return false;
        if (!Boolean.TRUE.equals(invoice.getClientValidated())) return false;
        return invoice.getStatus() != InvoiceStatus.VALIDATED
                && invoice.getStatus() != InvoiceStatus.ACCOUNTED
    ...
    private boolean isPendingBankStatement(BanqueReleve statement) {
        if (statement == null || statement.getStatus() == null) return false;
        if (!statement.isClientValidated()) return false;
        return statement.getStatus() != BanqueStatus.VALIDATED
                && statement.getStatus() != BanqueStatus.COMPTABILISE
                && statement.getStatus() != BanqueStatus.ERROR
                && statement.getStatus() != BanqueStatus.DUPLIQUE;
    }
